...
def add_arguments(parser):
    ...
    arg('-i', '--index', default=None, help='Index of the GAF file. If not provided, it is assumed to be in the same directory as GAF file with the same name and .gaf.gai extension')
    arg('-o', '--output', default=sys.stdout, help='Output Indexed GAF file. If omitted, use <GAF File>.gai.')
    arg('--node', dest='node', metavar='NODE', default=[], action='append', help='Give nodes to output alignments containing all of them. Instead of node ID, regions can also be specified. Can be used multiple times. When multiple nodes are specified, output contains only thenodes between the first and last nodes. Entire alignment can be shown with --full-alignment tag.')
